// dogme-core.js - Core helper & UI glue for Dogme site
...
const LevelSystem = {
  ...
  getLevelInfo() {
    const exp = Number.parseFloat(localStorage.getItem("dogme_user_exp") || "0")
  ...
  updateUI() {
    ...
    if (homeBadge) {
      if (isLoggedIn) {
        ...
        if (tag) {
          tag.innerText = `V${Math.floor(info.min / 10)}`
        ...
        const nameText = document.getElementById("rank-name-text")
        if (nameText) {
...
const DogmeCore = {
  ...
  handleUserRoute() {
    ...
    window.location.href = isLogged ? "user.html" : "login.html"
  },
